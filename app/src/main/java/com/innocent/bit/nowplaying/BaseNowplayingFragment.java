...
import com.innocent.bit.activities.BaseActivity;
import com.innocent.bit.adapters.BaseQueueAdapter;
import com.innocent.bit.adapters.SlidingQueueAdapter;
import com.innocent.bit.dataloaders.QueueLoader;
import com.innocent.bit.listeners.MusicStateListener;
import com.innocent.bit.timely.TimelyView;
import com.innocent.bit.utils.Helpers;
import com.innocent.bit.utils.PreferencesUtility;
import com.innocent.bit.utils.SlideTrackSwitcher;
import com.innocent.bit.utils.BitUtils;
import com.innocent.bit.widgets.CircularSeekBar;
import com.innocent.bit.widgets.DividerItemDecoration;
import com.innocent.bit.widgets.PlayPauseButton;
import com.innocent.bit.widgets.PlayPauseDrawable;
...
public class BaseNowplayingFragment extends Fragment implements MusicStateListener {
        ...
        public void run() {
            ...
            if (mProgress != null) {
                ...
                if (elapsedtime != null && getActivity() != null)
                    elapsedtime.setText(BitUtils.makeShortTimeString(getActivity(), position / 1000));
            }
            overflowcounter--;
    ...
    public Runnable mUpdateElapsedTime = new Runnable() {
        ...
        public void run() {
            if (getActivity() != null) {
                String time = BitUtils.makeShortTimeString(getActivity(), MusicPlayer.position() / 1000);
                if (time.length() < 5) {
    ...
    public void setSongDetails(View view) {
        ...
        if (playPauseFloating != null) {
            playPauseDrawable.setColorFilter(BitUtils.getBlackWhiteColor(accentColor), PorterDuff.Mode.MULTIPLY);
    ...
    public void updateSongDetails() {
        ...
        if (!duetoplaypause) {
            if (albumart != null) {
                ImageLoader.getInstance().displayImage(BitUtils.getAlbumArtUri(MusicPlayer.getCurrentAlbumId()).toString(), albumart,
        ...
        if (songduration != null && getActivity() != null)
            songduration.setText(BitUtils.makeShortTimeString(getActivity(), MusicPlayer.duration() / 1000));
