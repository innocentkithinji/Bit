...
import com.innocent.bit.dataloaders.AlbumLoader;
import com.innocent.bit.dataloaders.AlbumSongLoader;
import com.innocent.bit.dataloaders.ArtistAlbumLoader;
import com.innocent.bit.dataloaders.ArtistLoader;
import com.innocent.bit.dataloaders.ArtistSongLoader;
import com.innocent.bit.dataloaders.PlaylistLoader;
import com.innocent.bit.dataloaders.PlaylistSongLoader;
import com.innocent.bit.dataloaders.SongLoader;
import com.innocent.bit.models.Album;
import com.innocent.bit.models.Artist;
import com.innocent.bit.models.Playlist;
import com.innocent.bit.models.Song;
...
public class WearBrowserService extends MediaBrowserService {
    ...
    private final class MediaSessionCallback extends MediaSession.Callback {
        ...
        public void onPlayFromMediaId(final String mediaId, Bundle extras) {
            ...
            setSessionActive();
            MusicPlayer.playAll(mContext, new long[]{songId}, 0, -1, BitUtils.IdType.NA, false);
        }
                    ...
                    switch (Integer.parseInt(Character.toString(parentId.charAt(0)))) {
                        case TYPE_ARTIST:
                            ...
                            for (Artist artist : artistList) {
                                String albumNmber = BitUtils.makeLabel(mContext, R.plurals.Nalbums, artist.albumCount);
                                String songCount = BitUtils.makeLabel(mContext, R.plurals.Nsongs, artist.songCount);
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ARTIST_SONG_ALBUMS) + Long.toString(artist.id), artist.name, Uri.parse("android.resource://" +
                                        "naman14.timber/drawable/ic_empty_music2"), BitUtils.makeCombinedString(mContext, albumNmber, songCount), MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            }
                            break;
                        case TYPE_ALBUM:
                            List<Album> albumList = AlbumLoader.getAllAlbums(mContext);
                            for (Album album : albumList) {
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ALBUM_SONGS) + Long.toString(album.id), album.title, BitUtils.getAlbumArtUri(album.id), album.artistName, MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            }
                            break;
                        case TYPE_SONG:
                            List<Song> songList = SongLoader.getAllSongs(mContext);
                            for (Song song : songList) {
                                fillMediaItems(mediaItems, String.valueOf(song.id), song.title, BitUtils.getAlbumArtUri(song.albumId), song.artistName, MediaBrowser.MediaItem.FLAG_PLAYABLE);
                            }
                            break;
                        ...
                        case TYPE_ARTIST_SONG_ALBUMS:
                            ...
                            for (Album album : artistAlbums) {
                                String songCount = BitUtils.makeLabel(mContext, R.plurals.Nsongs, album.songCount);
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ALBUM_SONGS) + Long.toString(album.id), album.title, BitUtils.getAlbumArtUri(album.id), songCount, MediaBrowser.MediaItem.FLAG_BROWSABLE);
                        ...
                        case TYPE_ARTIST_ALL_SONGS:
                            ...
                            for (Song song : artistSongs) {
                                fillMediaItems(mediaItems, String.valueOf(song.id), song.title, BitUtils.getAlbumArtUri(song.albumId), song.albumName, MediaBrowser.MediaItem.FLAG_PLAYABLE);
                            }
                            break;
                        case TYPE_PLAYLIST:
                            List<Playlist> playlistList = PlaylistLoader.getPlaylists(mContext, false);
                            for (Playlist playlist : playlistList) {
                                String songCount = BitUtils.makeLabel(mContext, R.plurals.Nsongs, playlist.songCount);
