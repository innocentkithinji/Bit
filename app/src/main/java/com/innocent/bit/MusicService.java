...
import com.innocent.bit.helpers.MediaButtonIntentReceiver;
import com.innocent.bit.helpers.MusicPlaybackTrack;
import com.innocent.bit.lastfmapi.LastFmClient;
import com.innocent.bit.lastfmapi.models.LastfmUserSession;
import com.innocent.bit.lastfmapi.models.ScrobbleQuery;
import com.innocent.bit.permissions.Nammu;
import com.innocent.bit.provider.MusicPlaybackState;
import com.innocent.bit.provider.RecentStore;
import com.innocent.bit.provider.SongPlayCount;
import com.innocent.bit.utils.NavigationUtils;
import com.innocent.bit.utils.PreferencesUtility;
import com.innocent.bit.utils.BitUtils;
import com.innocent.bit.utils.BitUtils.IdType;
...
public class MusicService extends Service {
    ...
    private void addToPlayList(final long[] list, int position, long sourceId, BitUtils.IdType sourceType) {
    ...
    private void doAutoShuffleUpdate() {
        ...
        for (int i = 0; i < toAdd; i++) {
            ...
            mPlaylist.add(new MusicPlaybackTrack(mAutoShuffleList[idx], -1, BitUtils.IdType.NA, -1));
            notify = true;
        }
        if (notify) {
            notifyChange(QUEUE_CHANGED);
        ...
        if (mRemoteControlClient != null) {
            ...
            if (what.equals(META_CHANGED) || what.equals(QUEUE_CHANGED)) {
                ...
                if (mShowAlbumArtOnLockscreen) {
                    albumArt = ImageLoader.getInstance().loadImageSync(BitUtils.getAlbumArtUri(getAlbumId()).toString());
                    if (albumArt != null) {
    ...
    private Notification buildNotification() {
        ...
        Bitmap artwork;
        artwork = ImageLoader.getInstance().loadImageSync(BitUtils.getAlbumArtUri(getAlbumId()).toString());
        ...
        if (artwork != null && BitUtils.isLollipop())
                ...
                do {
                    TrackItem t = new TrackItem()
                            .setArt(ImageLoader.getInstance()
                                    .loadImageSync(BitUtils.getAlbumArtUri(c.getLong(c.getColumnIndexOrThrow(AudioColumns.ALBUM_ID))).toString()))
                            ...
                            .setDuration(BitUtils.makeShortTimeString(this, c.getInt(c.getColumnIndexOrThrow(AudioColumns.DURATION)) / 1000));
                    list.add(t.get());
                } while (c.moveToNext());
                try {
                ...
                try {
                    if (mCursor != null && shouldAddToPlaylist) {
                        ...
                        mPlaylist.add(new MusicPlaybackTrack(
                                mCursor.getLong(IDCOLIDX), -1, BitUtils.IdType.NA, -1));
                        notifyChange(QUEUE_CHANGED);
                        mPlayPos = 0;
                        mHistory.clear();
                    }
    ...
    public void open(final long[] list, final int position, long sourceId, BitUtils.IdType sourceType) {
        synchronized (this) {
            if (mShuffleMode == SHUFFLE_AUTO) {
...
//            }
    ...
    private static final class ServiceStub extends IBitService.Stub {
