...
import com.innocent.bit.adapters.SongsListAdapter;
import com.innocent.bit.dataloaders.LastAddedLoader;
...
import com.innocent.bit.dataloaders.TopTracksLoader;
...
import com.innocent.bit.widgets.DragSortRecycler;
...
public class PlaylistDetailActivity extends BaseActivity implements ATEActivityThemeCustomizer, ATEToolbarCustomizer {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (animate && BitUtils.isLollipop() && PreferencesUtility.getInstance(this).getAnimations()) {
    ...
    private void setAlbumart() {
        ...
        loadBitmap(BitUtils.getAlbumArtUri(getIntent().getExtras().getLong(Constants.ALBUM_ID)).toString());
    }
    ...
    private void setRecyclerViewAapter() {
        ...
        if (animate && BitUtils.isLollipop() && PreferencesUtility.getInstance(mContext).getAnimations()) {
    ...
    private void clearAutoPlaylists() {
        switch (action) {
            case Constants.NAVIGATE_PLAYLIST_LASTADDED:
                BitUtils.clearLastAdded(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_RECENT:
                BitUtils.clearRecent(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_TOPTRACKS:
                BitUtils.clearTopTracks(this);
                break;
        }
        Intent returnIntent = new Intent();
        setResult(Activity.RESULT_OK, returnIntent);
